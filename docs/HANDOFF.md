...
**It failed twice before that, and the cause is fixed.** `ERROR (invalid-state)` on every callback, including from fresh private windows: **Kinsta edge-cached the WPS Hide Login slug for 24 hours** (`s-maxage=86400`), freezing the OpenID button's one-time `state` — transient TTL 180 seconds — into the HTML and serving that dead value to everyone. **WPS Hide Login was removed and `OIDC_LOGIN_TYPE` set to `auto`.** `/wp-login.php` now builds a fresh authorization request server-side and redirects straight to Entra; verified as two different `state` values on consecutive requests with `x-kinsta-cache: BYPASS`. **No user sees a WordPress login form in either journey** — a deep link into a restricted area and the Canvas button both route through it. Full write-up at `PLAN.md` `4a`, deviation note in `REQUIREMENTS.md` §5.

**`employeeId` is absent from the ID token — the only open item on SSO.** Everything authentication needs arrived: `email`, `preferred_username`, `given_name`, `family_name`, `name`. Only Job 5's Canvas SIS linkage is affected. A note to Aidan is owed — `PLAN.md` `4b`. **Don't let anyone "fix" it by setting a userinfo endpoint**; Graph's `/oidc/userinfo` returns a smaller fixed set and would strip claims rather than add them.
...
**WPS Hide Login broke two things and was removed on 2026-08-18 — don't reinstall it.** Both failures share one cause, and it generalises: *a host's protections and its caching policy are keyed to the URLs it knows about. Moving a well-known URL forfeits both, and the caching loss stays invisible until something time-sensitive is served stale.*

1. **It removed Kinsta's brute-force ban**, which watches `/wp-login.php` and only that. Measured: `POST /wp-login.php` → 403 at the edge; `POST` to the custom path → 200, processed.
2. **It broke SSO.** Kinsta excludes `/wp-login.php` and `/wp-admin/` from caching; a custom slug is an ordinary public page, cached at `s-maxage=86400`. That froze the OpenID `state` — TTL 180 seconds — and served one dead nonce to every visitor for a day. Sign-in failed with `invalid-state` from fresh private windows, indistinguishable from a real misconfiguration.

**Diagnose this class of fault with two `curl`s, not by retrying in the browser.** Identical `state` values across independent requests with `x-kinsta-cache: HIT` is conclusive; `?nocache=<random>` returns `BYPASS` and a fresh state. **Note that 302s are cached on those paths too**, so auto-redirecting from a cached URL relocates the frozen nonce into a `Location` header rather than fixing anything.

**A single-use nonce cannot live in any stored URL.** The authorization URL the login page builds embeds a 180-second `state`. Copying it anywhere durable — a Canvas link, a bookmark, documentation — produces something that fails for everyone forever. **Always link to `/wp-login.php`**, which mints a fresh one server-side under `OIDC_LOGIN_TYPE=auto`.
