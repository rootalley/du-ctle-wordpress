...
- **Self-paced courses** with enrollment, quizzes, and completion tracking (via an LMS plugin)
- **Events calendar** with registration, capacity management, and waitlists
- **Discussion forums** restricted to authenticated DU faculty
- **Badge/credential system** (Open Badges 2.0)
- **Single Sign-On** via Microsoft Entra ID (OIDC/SAML)
- **LTI 1.3 integration** with Canvas LMS

The site will be managed day-to-day by a CTLE administrator (non-technical content author) and a contract developer. DU IT's involvement is limited to SSO configuration, DNS, and security vetting.
...
| H-3 | **MySQL 8.0+ or MariaDB 10.6+** database engine. | Required |
...
| H-10 | **Scalability** — ability to handle traffic surges (e.g., start-of-semester enrollment spikes) without manual intervention. Auto-scaling or burst capacity is preferred. | Preferred |

### 3.2 Storage & Transfer

| # | Requirement | Priority |
|---|---|---|
| S-1 | **Disk storage** — sufficient for the site's expected content (course materials, documents, images). Minimum 10 GB; 20+ GB preferred. Vendor should specify included storage and overage pricing. | Required |
...
| C-9 | **Brute-force protection** — login rate limiting, CAPTCHA support, or IP-based blocking for the WordPress admin panel. | Required |
| C-10 | **Two-factor authentication (2FA)** support for WordPress admin accounts (may be handled via plugin, but hosting must not block it). | Required |
...
| C-13 | **Vulnerability disclosure** — vendor must have a documented process for notifying customers of security incidents affecting their infrastructure. | Required |
| C-14 | **FERPA awareness** — while FERPA applicability is under review (see REQUIREMENTS.md §16 Open Question #7), vendor should be prepared to execute a data processing agreement or business associate agreement if required. | Preferred |
...
| # | Requirement | Priority |
|---|---|---|
| W-1 | **No plugin restrictions** — ability to install any WordPress plugin from the official repository or from third-party sources (zip upload). Some budget managed hosts restrict custom plugins; this is not acceptable. | Required |
...
| W-3 | **Cron job support** — reliable execution of WordPress cron (`wp-cron`) or support for server-level cron to replace WordPress's pseudo-cron. Server-level cron is preferred for reliable scheduled tasks (email notifications, event reminders, waitlist processing). | Required |
| W-4 | **Outbound email / SMTP** — ability to configure an external SMTP relay or transactional email provider (SendGrid, Mailgun, Amazon SES, etc.) for reliable email delivery. Port 587 and port 465 must not be blocked. | Required |
| W-5 | **REST API and external API calls** — no restrictions on outgoing HTTP/HTTPS requests from PHP (required for Microsoft Graph API, Entra SSO token endpoints, Panopto embeds, Open Badges API, etc.). | Required |
...
| W-7 | **PHP memory limit** — minimum 256 MB (LMS plugins with quiz engines and badge triggers can be memory-intensive). | Required |
| W-8 | **PHP `max_execution_time`** — minimum 120 seconds (for bulk enrollment operations and content imports). | Required |
| W-9 | **Database access** — phpMyAdmin or equivalent web-based database management tool, plus direct MySQL/MariaDB access for the developer if needed. | Preferred |
...
| P-2 | **Content Delivery Network (CDN)** — integrated or bundled CDN for static asset delivery (images, CSS, JS). Reduces latency for off-campus users. | Required |
| P-3 | **Server-side caching** — object caching (Redis or Memcached) and/or full-page caching built into the hosting stack. Must be compatible with WordPress plugins for cache invalidation. | Required |
| P-4 | **HTTP/2 or HTTP/3** support. | Required |
| P-5 | **Response time** — average server response time (TTFB) under 500 ms for uncached pages and under 200 ms for cached pages under normal load. | Preferred |
...
| P-7 | **Image optimization** — built-in or supported image compression/WebP conversion to improve page load performance. | Preferred |
...
| # | Requirement | Priority |
|---|---|---|
| K-1 | **Pricing transparency** — vendor must provide clear, all-inclusive pricing. Itemize any additional costs for overage (storage, bandwidth), premium support, CDN, backups, or staging environments. | Required |
...
| C-9 | Brute-force login protection | | |
| C-10 | 2FA support for admin accounts | | |
| C-11 | Access and error log availability | | |
| C-12 | Data return/destruction policy on termination | | |
| C-13 | Security incident notification process | | |
| C-14 | Willingness to sign DPA/BAA if needed | | |

### WordPress Platform

| # | Requirement | Y/P/N | Notes |
|---|---|---|---|
| W-1 | No restrictions on custom plugin installation | | |
| W-2 | Configurable PHP settings (memory, timeouts, uploads) | | |
| W-3 | Server-level cron support | | |
| W-4 | Outbound SMTP (ports 587/465 open) | | |
...
| W-9 | Database management tool (phpMyAdmin or equivalent) | | |
...
| P-2 | Integrated CDN | | |
| P-3 | Server-side caching (Redis/Memcached/full-page) | | |
| P-4 | HTTP/2 or HTTP/3 | | |
| P-5 | TTFB < 500 ms uncached, < 200 ms cached | | |
| P-6 | Resource isolation (container/dedicated) | | |
| P-7 | Image optimization (compression/WebP) | | |
...
| # | Requirement | Y/P/N | Notes |
|---|---|---|---|
| K-1 | Transparent, all-inclusive pricing | | |
...
| Security & Compliance (C) | /11 | 11 | /3 | 3 |
| WordPress Platform (W) | /7 | 7 | /2 | 2 |
| Performance & Reliability (P) | /4 | 4 | /3 | 3 |
| Support (T) | /5 | 5 | /2 | 2 |
