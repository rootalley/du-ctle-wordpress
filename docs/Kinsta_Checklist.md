...
| 0.1.1 | 2026-07-24 | sendres | Corrected the plugin name from "LTI Platform for WordPress" to **LTI Tool** (ceLTIc, plus its **ceLTIc LTI Library** dependency); WordPress is the LTI tool launched from Canvas. Terminology fix applied at the owner's request despite this being a historical evaluation record; the compatibility conclusion is unchanged. See IT_REQUESTS.md Request 3. |
