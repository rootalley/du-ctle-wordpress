...
The Kinsta account, billing, and site provisioning are complete. DNS for `ctle.dom.edu` has been delivered by DU IT. WordPress infra/plugin configuration is substantially done (§4–§12; see the 2026-07-28 build session). **The critical path is now entirely external:** Entra SSO and the M365 mailbox are specified and requested after the 2026-07-27 IT meeting (SSO Option 1; `ctle-noreply@dom.edu` sender; Graph `Mail.Send`) but not yet provisioned by IT. **Canvas LTI was dropped 2026-07-28** (decision 10) in favor of a Canvas global-nav link + Entra SSO, removing an integration workstream — so **SSO (IT-1) is now the single remaining launch integration**, and it gates forums, admin elevation, and the Canvas launch link alike.

| Group | Sections | Status |
|---|---|---|
| Initial Setup | §1–§3 | ✅ Complete |
| WordPress Configuration | §4–§8 | 🟡 §4 partial (core 7.0.2 done; sample content pending CD-N5); §5 **plugin installs done 2026-07-28** (Hide Login, WP Activity Log, Query Monitor active; WP Mail SMTP pre-staged; **LTI Tool + ceLTIc deactivated — LTI superseded by nav-link + SSO, decision 10**; Relevanssi staged inactive; alerting via mu-plugin, delivery pending IT-2); §6–§8 substantially done (baseline 07-27; XML-RPC off, open-registration off 07-28) — password admin eliminated, SSH recovery verified |
| Infrastructure | §9–§12 | 🟢 **§9 complete** — site live at `https://ctle.dom.edu`, HTTPS enforced, URLs cut over, verified 07-24. **§10–§12 done 2026-07-28** (PHP 8.4 + limits + cron; CDN + Polish + alerts + auth BYPASS; backups verified) — except the off-site 30-day backup (ME-11, post-launch). |
...
| ME-4 | Steven | §5 — install WPS Hide Login, WP Activity Log, Query Monitor | 2026-07-24 | ✅ Done 2026-07-28 — all three installed + active on Live (WPS Hide Login 1.9.18, WP Activity Log 5.6.5, Query Monitor 4.0.7). Admin-login / role-change **alerting** implemented via the free `mu-plugins/ctle-admin-alerts.php` must-use plugin (WP Activity Log's custom notifications are Premium-only); email **delivery** pending IT-2 (Graph). Also pre-staged on Live: WP Mail SMTP 4.9.0, LTI Tool 3.2.6 + ceLTIc LTI Library 5.3.2 (config pending §15/§16). |
