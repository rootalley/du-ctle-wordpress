...
> 🔴 **Superseded the same day — 2026-07-29, environment-divergence finding.** "Nothing is waiting on us" is **no longer true.** Live and Staging have each accumulated state the other lacks: Live holds the entire security and infrastructure build (CD-2 put it there), while Staging holds the Developer's theme and content work. **There is no safe push in either direction** — staging→live destroys the security build, live→staging destroys the Developer's work. Worse, Kinsta pushes **environment settings unconditionally, even on a files-only or database-only push**, so the usual "push files only" safeguard does not protect PHP 8.4 (§10) or redirect-to-primary (§9). New work is now open on our side: **CD-N7, ME-16, ME-17, ME-18**. Full mechanics and the push protocol: `kinsta_onboarding.md` §24. The paragraph below describes the state *before* this finding and is retained for context.
...
| CD-N7 | **Do not push Staging to Live.** | 🔴 **Send today — do not batch this one.** The Developer has a "Push to Live" button in MyKinsta that would undo the entire security and infrastructure build, and **nothing in her experience would flag it as dangerous** — pushing staging to live is the normal deployment step on essentially every other host, and on Kinsta generally. This site is the exception only because CD-2 put the build on Live *after* Staging was cloned. The risk is not carelessness; it is a correct habit meeting an unusual configuration. Must reach her **before** the merge (ME-18) and before any further staging work. | ⬜ **Not sent — urgent** |
...
| ME-16 | Steven | **Align Staging's environment settings to Live's** — PHP **8.4** and the redirect-all-to-primary rule. Two payoffs from one change: it removes the compatibility gap (the Developer is otherwise building against a different PHP version than Live runs), and it neutralizes the unavoidable half of a Kinsta push, since environment settings overwrite the destination **even on a files-only or database-only push**. Once the two environments match, that forced overwrite writes the values it replaces. | Before ME-18 | ⬜ **Open — do first.** Prerequisite for any push in either direction. |
| ME-17 | Steven | **Ask Kinsta support one question:** does a file push *delete* files in the destination that are absent from the source, or only add and overwrite? Undocumented, and it decides whether a push silently removes `ctle-admin-alerts.php` from Live — it was deployed to Live only, while `ctle-hardening.php` went to both. Interim mitigation regardless of the answer: put **both** mu-plugins on Staging so the question stops mattering. | Before ME-18 | ⬜ Open |
| ME-18 | Steven | **Merge the Developer's Staging work onto Live** (replaces the "refresh Staging from Live" plan, which is void now that Staging holds real work). Draft shape, to be scoped against what she actually built: selective **file** push of `wp-content/themes/<theme>/` and `wp-content/uploads/`; selective **table** push of `wp_posts`, `wp_postmeta`, `wp_terms`, `wp_term_*`. **Never push** `wp_options` (holds `siteurl`, `blog_public`, `active_plugins`, the rotated login path) or `wp_users`/`wp_usermeta` (holds the `topsecretuser` deletion and the ME-10 `sis_user_id` stamping). Theme mods and widget config live in `wp_options` and must be **re-applied by hand on Live**. A page builder or any plugin writing its own tables changes the table list — confirm before executing. Run the §24 pre- and post-push checklists around it. | After CD-N7, ME-16, ME-17 | ⬜ Open — needs scoping against the Developer's actual build |
...
| CD-2 | Developer | Build environment decision | 2026-07-28 | ✅ Decided 2026-07-27 — build SSO/LTI on Live/production; staging reserved for post-launch update testing. SSO is hostname-bound and a staging→live push overwrites Live, so building on Live means IT registers one Entra redirect URI, not two. **Corollary added 2026-07-29 — the decision stands, but its consequence was never written down:** putting the build on Live makes **Live the source of truth**, so sync flows live→staging and a staging→live push is a *destructive* operation on this site, not a routine deployment. Left unstated, that gap put two build-on-staging instructions in `kinsta_onboarding.md` (§5, §23) directly at odds with this decision for two days — either would have destroyed the build if followed. Both corrected 07-29; protocol now in §24. |
...
| 0.2.0 | 2026-07-29 | sendres | **Environment-divergence finding — reopens work on our side.** Verified against Kinsta's Push Environments docs that a push carries **environment settings (redirects, geolocation, PHP version, Nginx config) unconditionally, even on a files-only or database-only push**, so the standard "push files only" safeguard does *not* protect §9's redirect-to-primary or §10's PHP 8.4. Confirmed excluded: .htpasswd, bot protection, the destination's custom Nginx config, domains/SSL; target is auto-backed-up first. With CD-2 having put the build on Live and the Developer now building in Staging, **neither push direction is safe** — staging→live destroys the security build, live→staging destroys her work. Added **CD-N7** (urgent: tell the Developer not to push — the button looks routine and is not), **ME-16** (align Staging's env settings to Live's), **ME-17** (ask Kinsta whether a file push deletes destination-only files, which decides whether `ctle-admin-alerts.php` survives), **ME-18** (merge Staging's theme/content onto Live, replacing the now-void "refresh Staging from Live" plan). Recorded the CD-2 corollary that was missing: Live is the source of truth, so sync flows live→staging. Cross-doc: `kinsta_onboarding.md` 0.8.0 — new **§24 Environment Push Protocol**, plus reversal of two stale build-on-staging instructions (§5, §23) that contradicted CD-2 and would have destroyed the build if followed. |
