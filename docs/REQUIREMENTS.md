...
| Role | Capabilities |
|---|---|
| **CTLE Admin** | Full site administration: create/edit/publish all content, manage courses and enrollments, manage events, moderate forums, configure achievements and engagement settings, manage faculty users, approve contributed content. |
...
| **Faculty (authenticated)** | View protected content, self-enroll in courses, complete assessments, participate in forums, RSVP/register for events, earn achievements. |
...
Each authenticated user has a WordPress profile consisting of (a) an account record storing identity and activity data, and (b) a self-view dashboard where the user can see their own course progress, earned achievements, and forum reputation. Profiles are not public — there is no browsable member directory, and no faculty member can view another's profile page.

### Profile Data Model

| Field | Source | Visibility | Editable by user |
|---|---|---|---|
| **Display name** | Entra claim / LTI launch payload, refreshed on every login | Shown publicly on forum posts, completion certificates, and achievement displays | No |
...
| **Cross-platform achievements** | Generated by GamiPress from course completions, event attendance, forum milestones, and custom triggers (see §8) | Self-view only | N/A |
...
- The dashboard displays the user's enrolled courses with progress indicators, completed courses with links to their PDF certificates, earned GamiPress achievements, and their current wpForo forum reputation level.
- Achievements are displayed in reverse chronological order (most recently earned first). Faculty cannot reorder, hide, or delete earned achievements.
...
- Integration with achievement and engagement plugins (see §8)
...
The primary goal of the achievements system is to **drive faculty engagement within the CTLE site** — encouraging course completion, event attendance, and forum participation through visible recognition and progress indicators. External credentialing (e.g., Open Badges / LinkedIn sharing) is a future consideration, not a launch requirement.

### Three-Layer Engagement Model

| Layer | What it rewards | Plugin | How it's visible |
|---|---|---|---|
| **Forum reputation** | Posting, replying, and receiving likes in discussion forums | wpForo (built-in reputation system) | Reputation title and level displayed on every forum post, visible to all forum participants |
| **Course achievements & certificates** | Completing courses and passing assessments | LMS plugin (built-in achievement/certificate system) | Achievement displayed on the faculty member's self-view dashboard; PDF certificate generated on completion (see §7) |
| **Cross-platform achievements** | Milestones that span multiple activities: event attendance, combined course + forum + event triggers, and custom achievements defined by CTLE Admin | GamiPress | Achievement notifications on completion; achievements displayed on the self-view dashboard |
...
- Reputation level and title are displayed on every forum post, providing social proof and motivating continued participation.
- No additional plugin or configuration is required beyond wpForo's default settings; CTLE Admin can customize level names and point thresholds.

### Course Achievements (LMS Plugin)

- The chosen LMS plugin's built-in achievement system handles course-completion recognition.
- PDF certificates are generated on completion per §7 (faculty name, course title, completion date, DU/CTLE branding, record ID).
- Course progress and completed courses are displayed on the faculty member's self-view dashboard (see Faculty Profiles under §5).

### Cross-Platform Achievements (GamiPress)

| Requirement | Detail |
|---|---|
| **Plugin** | GamiPress (free). |
| **Cost** | Free core with optional paid add-ons. The free version covers all launch requirements. |
| **Triggers** | Event attendance milestones (e.g., "attended 5 events"), cross-system achievements (e.g., "completed a course AND posted in a forum AND attended an event"), and custom achievements defined by CTLE Admin. |
| **Integration** | GamiPress has explicit, maintained integrations with wpForo, LearnDash, LifterLMS, Tutor LMS, and The Events Calendar — the same plugins in the CTLE stack. The developer should verify that these integrations are functional with the specific plugin versions selected. |
...
| **On-site display** | Earned achievements are displayed on the faculty member's self-view dashboard alongside course progress and forum reputation. |

### Future Consideration: Open Badges

If CTLE later decides to make achievements shareable externally (LinkedIn, email, personal portfolio sites), GamiPress offers an Open Badges add-on that can be enabled without restructuring the existing achievement system. This is deferred to post-launch evaluation (see §18).
...
- Forum participation is tracked for engagement awards via wpForo's built-in reputation system (see §8) and GamiPress cross-platform achievements.

### Recommended Plugin Approach

- **wpForo** (free core, actively maintained). wpForo uses its own database tables for better performance than WordPress's native `wp_posts` storage, provides modern forum layouts, built-in moderation tools, and a built-in reputation/badge system that drives forum engagement (see §8).
- wpForo's reputation system must be configured to display reputation levels on forum posts.
- Must integrate with GamiPress for cross-platform achievement triggers (e.g., "posted in 10 forum topics" as a GamiPress achievement trigger).
- Must respect the SSO-provisioned user accounts and display the Entra/LTI-synced display name and avatar.
...
The CTLE site collects personal data about DU faculty: identity (name, email, employee ID, avatar), training completion records, course enrollment and progress, forum activity, event registrations, earned achievements, and search query history. This section establishes the privacy obligations that apply to that data.
...
| Capability | Detail |
|---|---|
| **Self-view** | Faculty can view their own profile, completion records, enrolled courses, and earned achievements via the self-view dashboard (see Faculty Profiles under §5). |
...
| **Forums** | wpForo (free core) | Actively maintained; uses own database tables for better performance; built-in reputation system for forum engagement (see §8, §10). Replaces bbPress, which is in maintenance mode since 2020. |
...
|---|---|---|
| 1 | **GamiPress–wpForo integration verification:** Developer to verify that GamiPress's wpForo integration correctly detects forum activity (posts, replies, likes) as achievement triggers with the specific wpForo version deployed. See §8. | Open |
...
| 16 | **Open Badges for external sharing:** If CTLE later wants faculty to share achievements externally (LinkedIn, personal portfolios), GamiPress offers an Open Badges add-on. Evaluate post-launch based on faculty interest. See §8. | Future consideration |
