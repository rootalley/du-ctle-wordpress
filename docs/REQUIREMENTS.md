...
>
> **✅ Accepted by DU IT, 2026-08-18.** They are satisfied that obfuscating the login path adds nothing once users are directed to Entra for authentication. The protection model they have signed off is: MyKinsta 2FA on all company users, audit-log alerting on every Administrator login and role change, Entra-enforced MFA on the only interactive path, and no password-authenticated account on the site. **No further action, and this deviation is closed** — it remains recorded because the requirement row above still reads as approved.
>
> **Cleanup done 2026-08-18:** the orphaned `whl_page` option was deleted. Live carries no trace of the plugin.
...
- ✅ **Done 2026-08-18.** IT security sign-off obtained on the administrator access protection model as the agreed substitute for broader 2FA or IP-allowlist requirements on privileged access: MyKinsta 2FA on all company users, audit-log alerting on every Administrator login and role change, Entra-enforced MFA on the only interactive sign-in path, and no password-authenticated account on the site. **The obfuscated login URL was removed from this model** — DU IT accepted that it adds nothing once users are directed to Entra. See the deviation note in §5.
...
| 0.3.2 | 2026-08-18 | sendres | §5: **DU IT accepted the *Administrator access protection* item (2) deviation the same day it was raised** — they agree obfuscating the login path adds nothing once users are directed to Entra. The deviation is closed and the §*IT Responsibilities* security sign-off is obtained, covering MyKinsta 2FA, audit-log alerting, Entra-enforced MFA and the absence of any password-authenticated account. The orphaned `whl_page` option was deleted from Live. No requirement changed. |
